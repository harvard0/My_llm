"""
MizukiMind LoRA (Low-Rank Adaptation) 微调脚本
...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            Logger(
                f"Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}) loss:{current_loss:.4f} lr:{current_lr:.8f} epoch_Time:{eta_min}min:"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MizukiMind LoRA Fine-tuning")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="MizukiMind-LoRA", help="wandb项目名"
    )
    ...
    # 📚 模型配置初始化
    # MizukiMindConfig: 定义模型的超参数，如隐藏维度、层数、是否使用MoE
    ...
    if args.use_wandb and is_main_process():
        ...
        # 📚 实验名称生成
        # 包含关键参数，便于识别不同的实验配置
        wandb_run_name = f"MizukiMind-LoRA-{'MoE' if args.use_moe else ''}-{args.lora_name}-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
